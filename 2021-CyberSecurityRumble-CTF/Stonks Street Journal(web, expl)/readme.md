...
We can see an invalid syntax error where part of the statement is revealed. We know from SQL that the section preceeding the 'WHERE' clause is the 'FROM' section and most likely the table name. Given what I know from some quick PostGres DB doc pages, all new DB's contain a 'public' schema and all tables and objects are added to this by default. The syntax for this is schema.tableName so from this image i can safely assume that the table name is news_subscriber belonging to the public schema.

I'm gathering some great info here but doing this all manually through Cyberchef is quite painful and so i sought some help from a friend to create a python script to send my crafted queries and grep the results. The python script uses the requests library to send the URI including the base64 encoded SQLi command. The script can be found the challenge folder. I crafted some queries containing the 7 required fields as indicated by the earlier error to probe what data was required where and how i could retrieve some info. Of the 7 columns supplied, i learnt that the 2nd column was the one that would return the data i was asking for.
